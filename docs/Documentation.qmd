...
![K-means behavior illustrated on simple synthetic datasets with different shapes.](images/kmeans_behavior_01.png){#fig-background-03}

![K-means behavior illustrated on simple synthetic datasets with different shapes.](images/kmeans_behavior_02.png){#fig-background-04}
...
![A diagram of a feedforward artificial neural network illustrating the flow of data through input, hidden, and output layers.](./images/ann.png){#fig-background-05 width=80%}
...
Besides, a novel, the so called rhizome layer, is added in @fig-som-enhancement-02-02, to visualize the edges between $1st$ and $2nd$ BMU pairs implied by the data points: all connections and only those violating the neighborhood constraint, in each subplot, respectively.
